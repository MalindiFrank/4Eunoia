...
import { Lightbulb, BrainCircuit, Calendar as CalendarIcon, Activity, BarChartHorizontalBig, Wallet, ListTodo, AlertCircle, Smile, Scale, Flame, Zap, Loader2, Map, Mic } from 'lucide-react';
...
const InsightsPageSkeleton: FC = () => {
  return (
    <div className="container mx-auto p-4 md:p-6 lg:p-8 space-y-8">
      <Skeleton className="h-10 w-3/4 mb-6 rounded-md" /> {/* Title: AI Insights & Coaching */}
      
      {/* Daily Suggestions Card Skeleton */}
      <Card className="shadow-md">
        ...
        <CardContent className="space-y-3">
          ...
          <Skeleton className="h-8 w-1/4 mt-2 rounded-md" /> {/* Refresh button */}
        </CardContent>
      </Card>

      {/* Generate Specific Insights Card Skeleton */}
      <Card className="shadow-md">
        <CardHeader>
          <CardTitle><Skeleton className="h-6 w-1/2 rounded-md" /></CardTitle>
          <CardDescription><Skeleton className="h-4 w-3/4 rounded-md mt-1" /></CardDescription>
        ...
        <CardContent className="space-y-6">
          ...
          <Skeleton className="h-10 w-1/3 rounded-md" /> {/* Generate Button */}
        </CardContent>
      </Card>

      {/* Placeholder for a results card skeleton */}
      <Card className="shadow-md">
        <CardHeader>
...
const InsightsPageClient: FC = () => {
  ...
  const form = useForm<InsightsRequestFormValues>({
    ...
    defaultValues: {
      insightType: 'dailySuggestion', // Default to dailySuggestion
   ...
   const fetchDailySuggestions = useCallback(async () => {
        ...
        try {
             ...
             const tomorrow = endOfDay(subDays(now, -1)); // Corrected: end of today
    ...
    useEffect(() => {
        // Initialize with daily suggestions
        ...
        switch (insightType) {
             ...
             case 'burnoutRisk':
                  const burnoutInput: EstimateBurnoutRiskInput = {
                     ...dateInput,
                     dailyLogs: formatArrayForFlow(logsInRange, ['date']),
                     tasks: formatArrayForFlow(allTasks, ['createdAt', 'dueDate']), // Use allTasks for burnout risk
                     calendarEvents: formatArrayForFlow(eventsInRange, ['start', 'end']),
                  };
   ...
   useEffect(() => {
        if (toolParam) {
            ...
            // Automatically submit the form or fetch suggestions for the toolParam
            if (toolParam === 'dailySuggestion') {
                fetchDailySuggestions();
            } else {
                 onSubmit({ insightType: toolParam, startDate: effectiveStartDate, endDate: effectiveEndDate });
            }
            // Clean the URL query parameter after processing
                    ...
                    <Select onValueChange={(value: AIServiceType) => {
                        field.onChange(value);
                         ...
                         if (['productivity', 'expenseTrends', 'taskCompletion', 'sentimentAnalysis', 'lifeBalance', 'burnoutRisk'].includes(value)) {
                             ...
                             form.setValue('startDate', startOfDay(now), { shouldValidate: true }); // Plan for today
...
// The new Page component that will be the default export
